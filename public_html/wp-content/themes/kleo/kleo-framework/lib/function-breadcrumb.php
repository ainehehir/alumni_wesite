...
function breadcrumb_trail_get_buddypress_items($args = array()) 
{
	...
	if ( !empty( $bp->displayed_user->fullname ) ) { // looking at a user or self
		...
		$trail[] = '<a href="'.  home_url().'/'. $bp->current_component .'" title="'. esc_attr( get_the_title() ) .'">'. ucwords( $bp->current_component ) .'</a>';
		$trail[] = '<a href="'. get_permalink() . $bp->current_item .'" title="'.esc_attr($bp->bp_options_title).'">'. $bp->bp_options_title .'</a>';		
                
	} else if ( $bp->is_directory ) { // this is a top level directory page
		$trail_end = '<a href="'.  home_url().'/'. $bp->current_component .'" title="'. ucwords( $bp->current_component ) .'">'. ucwords( $bp->current_component ) .'</a>';
