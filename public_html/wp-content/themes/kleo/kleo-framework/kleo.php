    ...
    public function add_google_fonts_link()
    {
			...
			*/
			...
			if (get_transient(KLEO_DOMAIN.'_google_link') === FALSE)
			{
				...
				foreach ($sections as $section) 
				{
					...
					if (isset($font['subsets']) && !empty($font['subsets'])) {
						$fonts[$font['font-family']]['subset'][$font['subsets']] = $font['subsets'];
					}
					/*
			...
			if (isset($google_link) && $google_link != ''){
				...
				add_action( 'wp_enqueue_scripts', create_function( '', "wp_register_style('kleo-google-fonts', '//fonts.googleapis.com/css?family=Roboto Condensed:400,300,700' , array(), '', 'all' ); wp_enqueue_style('kleo-google-fonts');" ) ); 
			}
