...
function kleo_setup() {
	...
	add_image_size( 'kleo-full-width', 1038, 583, true );
	/*
	...
	/* Specific framework functionalities */
	add_theme_support( 'kleo-sidebar-generator' );
	add_theme_support( 'kleo-facebook-login' );
    
	/* Third-party plugins */
	//add_theme_support( 'buddypress' ); /* no longer needed for theme compat */
		...
		<div id="respond-wrap">
			<?php 
				...
				$comments_args = array(
						...
						'logged_in_as'		   => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'kleo_framework' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '</p>',
...
function kleo_switch_layout($layout=false)
{
	...
	switch ($layout) {
		...
		case 'left':
			...
			add_filter('kleo_main_template_classes', create_function('$cols', '$cols = "col-sm-9 col-sm-push-3 tpl-left"; return $cols;'));
			add_filter('kleo_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 sidebar-left col-sm-pull-9"; return $cols;'));			
			break;
		...
		case '3ll':
			...
			add_filter('kleo_main_template_classes', create_function('$cols', '$cols = "col-sm-6 col-sm-push-6 tpl-3ll"; return $cols;'));
			add_filter('kleo_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 col-sm-pull-6 sidebar-3ll"; return $cols;'));
			add_filter('kleo_extra_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 sidebar-3ll"; return $cols;'));
			break;
		
		case '3lr':
			add_action('kleo_after_content', 'kleo_sidebar');
			add_action('kleo_after_content', 'kleo_extra_sidebar');
			add_filter('kleo_main_template_classes', create_function('$cols', '$cols = "col-sm-6 col-sm-push-3 tpl-3lr"; return $cols;'));
			add_filter('kleo_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 col-sm-pull-6 sidebar-3lr"; return $cols;'));
			add_filter('kleo_extra_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 sidebar-3lr"; return $cols;'));
			break;
		...
		case '3rr':
			...
			add_filter('kleo_extra_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 sidebar-3rr"; return $cols;'));
			break;
		

		default:
			//default to right template
			add_action('kleo_after_content', 'kleo_sidebar');
			remove_action('kleo_after_content', 'kleo_extra_sidebar');
			add_filter('kleo_main_template_classes', create_function('$cols', '$cols = "col-sm-9 tpl-right"; return $cols;'));
			add_filter('kleo_sidebar_classes', create_function('$cols', '$cols = "col-sm-3 sidebar-right"; return $cols;'));
...
function kleo_prepare_layout() {
	...
	elseif(is_singular('post') && get_cfield('post_layout') && get_cfield('post_layout') != 'default' ) {
		...
		if (kleo_has_shortcode('vc_row') && $layout == 'no') {
