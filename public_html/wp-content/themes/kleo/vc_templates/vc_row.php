...
switch ($type) {
	...
	case 'image':
		...
		if ( $bg_image && !in_array( $bg_image, array('none') ) ) {
			$bg_image_path = wp_get_attachment_image_src($bg_image, 'kleo-full-width');
	...
	case 'video':
		...
		if ( !empty($bg_video_args) ) {
			...
			$bg_video .= sprintf( '<div class="video-wrap"><video %s controls="controls" class="full-video">', join( ' ', $attr_strings ) );
...
$output .= "\n".'<section class="'. esc_attr(trim(implode(' ', $section_classes))) .'"' .$data_attr . $style. '>';
